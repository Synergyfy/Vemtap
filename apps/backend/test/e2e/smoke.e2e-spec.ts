...
describe('Smoke Test (E2E)', () => {
  ...
  it('should ensure all registered routes return status < 500', async () => {
    ...
    if (!router) {
         console.warn('Could not access Express router. Smoke test cannot discover routes automatically.');
         if (expressApp) {
    ...
    const getRoutes = (stack: any[], basePath: string = ''): { path: string, method: string }[] => {
        ...
        stack.forEach((layer) => {
            if (layer.route) {
                ...
                // Recursively check sub-routers
                 ...
                 routes = routes.concat(getRoutes(layer.handle.stack, basePath + routePath));
            }
        });
        return routes;
    }
    ...
    if (availableRoutes.length === 0) {
        console.warn('No routes found automatically. Smoke test might be skipping routes.');
    }

    for (const route of availableRoutes) {
      // Replace parameter placeholders with dummy values
      // e.g. /users/:id -> /users/dummy-param
      const path = route.path.replace(/:[^\/]+/g, '123e4567-e89b-12d3-a456-426614174000'); // Use a UUID-like dummy
      ...
      try {
        ...
        if (res.status >= 500) {
            console.error(`Smoke test failed for ${route.method} ${path}: Status ${res.status}`, res.body);
        }

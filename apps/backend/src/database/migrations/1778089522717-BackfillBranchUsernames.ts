...
export class BackfillBranchUsernames1778089522717 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        for (const branch of branches) {
            ...
            let baseName = (branch.branch_name || branch.business_name || 'branch').toLowerCase();
            ...
            console.log(`Updated branch ${branch.id} with username: ${finalUsername}`);
        }
    }

    public async down(queryRunner: QueryRunner): Promise<void> {
        // Remove auto-generated usernames (set back to NULL)
        await queryRunner.query(`UPDATE branches SET username = NULL WHERE username IS NOT NULL`);
    }
}

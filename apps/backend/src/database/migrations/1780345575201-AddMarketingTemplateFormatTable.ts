...
export class AddMarketingTemplateFormatTable1780345575201 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `DO $$ BEGIN IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'marketing_categories') THEN ALTER TABLE "marketing_categories" ALTER COLUMN "slug" DROP DEFAULT; END IF; END $$;`,
    );
    await queryRunner.query(
      `DO $$ BEGIN IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'marketing_templates') AND EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'marketing_categories') THEN ALTER TABLE "marketing_templates" DROP CONSTRAINT IF EXISTS "FK_1ecbb98d1f43670477c7638df53"; END IF; END $$;`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `DO $$ BEGIN IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'marketing_brand_rules') AND EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'businesses') THEN ALTER TABLE "marketing_brand_rules" DROP CONSTRAINT IF EXISTS "FK_27eb70ba4fc6312e8ac269eea63"; END IF; END $$;`,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `DO $$ BEGIN IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'marketing_categories') THEN ALTER TABLE "marketing_categories" ALTER COLUMN "slug" SET DEFAULT ''; END IF; END $$;`,
    );
    await queryRunner.query(
      `ALTER TABLE "settings" ALTER COLUMN "messagingCostEmail" SET DEFAULT 0.01`,
    );
    ...
    await queryRunner.query(
      `DROP TABLE IF EXISTS "marketing_template_formats"`,
    );

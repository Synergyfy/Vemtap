...
export class AddContactFieldsToProfileLatest1775498698028 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE "business_profiles" ADD "contactEmail" character varying`,
    );
    await queryRunner.query(
      `ALTER TABLE "business_profiles" ADD "contactPhone" character varying`,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "business_profiles" DROP COLUMN "contactPhone"`,
    );
    await queryRunner.query(
      `ALTER TABLE "business_profiles" DROP COLUMN "contactEmail"`,
    );
  }
}

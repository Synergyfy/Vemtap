...
export class UpdateMessagingEntities1773822550968 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "conversation_threads" ADD "lastMessageContent" text`,
    );
    await queryRunner.query(
      `ALTER TABLE "conversation_threads" ADD "branchUnreadCount" integer NOT NULL DEFAULT '0'`,
    );
    await queryRunner.query(
      `ALTER TABLE "conversation_threads" ADD "customerUnreadCount" integer NOT NULL DEFAULT '0'`,
    );
    await queryRunner.query(
      `ALTER TABLE "contacts" ADD "pushToken" character varying`,
    );
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "conversation_threads" DROP COLUMN "customerUnreadCount"`,
    );
    await queryRunner.query(
      `ALTER TABLE "conversation_threads" DROP COLUMN "branchUnreadCount"`,
    );
    await queryRunner.query(
      `ALTER TABLE "conversation_threads" DROP COLUMN "lastMessageContent"`,
    );

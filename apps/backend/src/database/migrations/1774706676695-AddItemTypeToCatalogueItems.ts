...
export class AddItemTypeToCatalogueItems1774706676695 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TYPE "public"."catalogue_items_itemtype_enum" AS ENUM('product', 'service')`,
    );
    await queryRunner.query(
      `ALTER TABLE "catalogue_items" ADD "itemType" "public"."catalogue_items_itemtype_enum" NOT NULL DEFAULT 'product'`,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "catalogue_items" DROP COLUMN "itemType"`,
    );
    await queryRunner.query(
      `DROP TYPE "public"."catalogue_items_itemtype_enum"`,
    );
  }
}

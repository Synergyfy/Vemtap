...
export class DeviceTapController {
  ...
  @ApiResponse({ status: 200, description: 'Device context retrieved successfully' })
  async getContext(@Param('code') code: string) {
    const deviceWithRelations = await this.devicesService.findByCodeWithRelations(code);
  ...
  @Public()
  @Post('record/:code')
  @ApiOperation({ summary: 'Record a visit for an anonymous or identified user' })
  async recordVisit(
    @Param('code') code: string,
    @Body() dto: { visitorId?: string; name?: string; email?: string; phone?: string },
  ) {

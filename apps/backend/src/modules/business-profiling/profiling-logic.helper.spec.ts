...
describe('ProfilingLogicHelper', () => {
  describe('calculateRetail', () => {
    it('should correctly identify problems and recommendations for a high-problem retail business', () => {
      ...
      expect(result.insights.problems!).toContain(
        'Customers struggle to find or understand products easily.',
      );
      expect(result.insights.problems!).toContain(
        'High service delay noticed during customer interactions.',
      );
      expect(result.insights.problems!).toContain(
        'No structured customer database for re-marketing.',
      );
  ...
  describe('calculateFood', () => {
    it('should recommend QR ordering for busy restaurants with waiting issues', () => {
      ...
      expect(result.insights.problems!).toContain(
        'Service delays and customer wait-time friction.',
      );
      expect(result.insights.problems!).toContain(
        'Menu confusion and low offering visibility.',
      );

...
import { AdminCreateAgentDto, GenerateImpersonationTokenDto, GenerateCustomerImpersonationTokenDto } from './dto/administration.dto';
...
describe('AdministrationService', () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: getRepositoryToken(ImpersonationToken), useValue: mockTokenRepository },
        { provide: getRepositoryToken(CustomerImpersonationToken), useValue: mockCustomerTokenRepository },
    ...
    tokenRepository = module.get<Repository<ImpersonationToken>>(getRepositoryToken(ImpersonationToken));
    customerTokenRepository = module.get<Repository<CustomerImpersonationToken>>(getRepositoryToken(CustomerImpersonationToken));
  ...
  describe('createAgent', () => {
    it('should throw if email exists', async () => {
      ...
      await expect(service.createAgent({ email: 'test@example.com' } as AdminCreateAgentDto)).rejects.toThrow(BadRequestException);
    });
  ...
  describe('generateToken', () => {
    it('should throw if actor not found', async () => {
      mockUserRepository.findOne.mockResolvedValue(null);
      await expect(service.generateToken('actor', {} as GenerateImpersonationTokenDto)).rejects.toThrow(NotFoundException);
    });

    it('should throw if actor is not admin or agent', async () => {
      mockUserRepository.findOne.mockResolvedValue({ role: UserRole.CUSTOMER });
      await expect(service.generateToken('actor', {} as GenerateImpersonationTokenDto)).rejects.toThrow(BadRequestException);
    });

    it('should throw if expiresAt is in the past', async () => {
      mockUserRepository.findOne.mockResolvedValue({ role: UserRole.ADMIN });
      await expect(service.generateToken('actor', { expiresAt: '2000-01-01T00:00:00Z' } as GenerateImpersonationTokenDto)).rejects.toThrow(BadRequestException);
    });
    ...
    it('should throw if expiresAt exceeds MAX_HOURS', async () => {
      ...
      await expect(service.generateToken('actor', { expiresAt: future } as GenerateImpersonationTokenDto)).rejects.toThrow(BadRequestException);
    });
  ...
  describe('getActorPermissions', () => {
    it('should return implicitly full permissions for admin', async () => {
      mockUserRepository.findOne.mockResolvedValue({
        id: 'admin', role: UserRole.ADMIN, permissions: null
      });
    ...
    it('should return exact permissions for agent', async () => {
      mockUserRepository.findOne.mockResolvedValue({
        id: 'agent', role: UserRole.AGENT, permissions: [BackendModule.LOYALTY]
      });
  ...
  describe('generateCustomerToken', () => {
    it('should throw if target customer not found', async () => {
      ...
      await expect(service.generateCustomerToken('actor', { targetCustomerId: 'c' } as GenerateCustomerImpersonationTokenDto)).rejects.toThrow(NotFoundException);
    });
    ...
    it('should create and save customer token', async () => {
      ...
      const result = await service.generateCustomerToken('actor', { targetCustomerId: 'c', targetBranchId: 'b', expiresAt });

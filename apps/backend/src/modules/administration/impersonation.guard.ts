...
export class ImpersonationGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (user.id !== token.actorId) {
      throw new ForbiddenException('You are not the actor for this impersonation token');
    }
    ...
    if (!permissions.includes(BackendModule.ALL) && !permissions.includes(requiredModule)) {
      throw new ForbiddenException(`You do not have permission for the ${requiredModule} module`);
    }

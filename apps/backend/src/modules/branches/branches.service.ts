...
export class BranchesService {
    ...
    async create(ownerId: string, createBranchDto: CreateBranchDto): Promise<Branch> {
        ...
        // Check plan limits
        const capabilities = await this.subscriptionsService.getCapabilities(business.id);
        ...
        if (typeof branchLimit === 'number' && currentBranches >= branchLimit) {
            throw new ForbiddenException(`You have reached the branch limit for your plan (${branchLimit} branches)`);
        }
    ...
    async findOne(ownerId: string, id: string): Promise<Branch> {
        ...
        const branch = await this.branchesRepository.findOne({ where: { id, businessId: business.id } });
        if (!branch) {
    ...
    async update(ownerId: string, id: string, updateBranchDto: UpdateBranchDto): Promise<Branch> {

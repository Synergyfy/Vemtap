...
export class TwilioWebhookController {
  ...
  @ApiOperation({
    summary: 'Handle Twilio WhatsApp webhooks with signature validation',
  })
  async handleWebhook(
    @Body() payload: any,
    ...
    const isValid = this.twilioProvider.validateRequest(
      signature,
      url,
      payload,
    );
    ...
    this.logger.log(
      `Received Twilio webhook: ${payload.SmsSid || payload.MessageSid}`,
    );
    ...
    try {
      ...
      this.logger.error(
        `Failed to process Twilio webhook: ${error.message}`,
        error.stack,
      );

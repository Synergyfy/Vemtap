...
export class CreditPlanController {
    ...
    @ApiResponse({ status: 201, description: 'The credit plan has been successfully created.', type: CreditPlan })
    ...
    @ApiResponse({ status: 200, description: 'Return all active credit plans.', type: [CreditPlan] })
    findAll() {
    ...
    @ApiResponse({ status: 200, description: 'Return the credit plan details.', type: CreditPlan })
    ...
    @ApiResponse({ status: 200, description: 'The credit plan has been successfully purchased and credits awarded.', type: BusinessCredit })
    @ApiResponse({ status: 400, description: 'Payment verification failed or insufficient amount.' })
    purchase(
        @Param('id') id: string,
        @Body() purchaseDto: PurchaseCreditPlanDto,
    ) {
        return this.creditPlanService.purchase(purchaseDto.businessId, id, purchaseDto.reference);
    }
    ...
    @ApiResponse({ status: 200, description: 'The credit plan has been successfully updated.', type: CreditPlan })
    update(
        @Param('id') id: string,
    ...
    @ApiResponse({ status: 200, description: 'The credit plan has been successfully deactivated.' })

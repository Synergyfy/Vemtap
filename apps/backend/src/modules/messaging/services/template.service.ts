...
export class TemplateService {
    ...
    async createTemplate(businessId: string, name: string, channel: Channel, content: string): Promise<MessageTemplate> {
        this.validateFormat(channel, content);

        const existing = await this.templateRepo.findOne({ where: { businessId, name, channel } });
        if (existing) {
            throw new BadRequestException('Template with this name already exists for this channel.');
        }

        const template = this.templateRepo.create({ businessId, name, channel, content });
        return this.templateRepo.save(template);
    }

    async getTemplate(id: string, businessId: string): Promise<MessageTemplate> {
        const template = await this.templateRepo.findOne({ where: { id, businessId } });
        if (!template) {

...
export class MessagingFlowService {
    ...
    async create(dto: CreateFlowDto, user: User): Promise<Flow> {
        const businessId = user.role === UserRole.ADMIN ? dto.businessId : user.businessId;

        if (!businessId) {
            throw new BadRequestException('businessId is required');
        }

        const branchId = user.role === UserRole.ADMIN ? dto.branchId : (dto.branchId || user.branchId);
    ...
    async findAll(query: GetFlowsDto, user: User): Promise<Flow[]> {
        ...
        const filteredBusinessId = user.role === UserRole.ADMIN ? businessId : user.businessId;
        if (!filteredBusinessId) {
            throw new BadRequestException('businessId is required');
        }

        const filteredBranchId = user.role === UserRole.ADMIN ? branchId : (branchId || user.branchId);
    ...
    async updateStatus(id: string, dto: UpdateFlowStatusDto, user: User): Promise<Flow> {

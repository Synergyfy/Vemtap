...
export class BatchSendProcessor extends WorkerHost {
    ...
    async process(job: Job<BatchJobData, any, string>): Promise<any> {
        ...
        try {
            ...
            if (!business) {
                this.logger.error(`Business ${businessId} not found for campaign ${campaignId}`);
                return { successCount: 0, failureCount: 0 };
            }

            let template: any = null;
            if (templateId) {
                template = await this.templateService.getTemplate(templateId, businessId);
            }
            ...
            await this.campaignService.updateCampaign(campaignId, {
                ...
                actualCost: await this.messagingEngine.calculateCost(successCount, job.data.channel),
                sentAt: new Date(),
            });

            this.logger.log(`Completed batch send for campaign ${campaignId}. Success: ${successCount}, Failed: ${failureCount}`);
            return { successCount, failureCount };
        } catch (error) {
            this.logger.error(`Batch job failed completely for campaign ${campaignId}`, error.stack);
            await this.campaignService.updateCampaign(campaignId, {

...
export class PartnershipsController {
  ...
  @ApiOperation({
    ...
    description:
      'Finds active branches of other businesses within a radius that do not have active or pending partnerships.',
  })
  async findNearbyBranches(
    @Query() query: NearbyPartnersQueryDto,
    @Req() req: any,
  ) {
    return this.partnershipsService.findNearbyPartnerableBranches(
      query,
      req.user,
    );
  }
  ...
  @ApiOperation({
    ...
    description:
      'Allows the recipient branch to accept or decline the partnership invitation.',
  })
  async respond(
    @Param('id') id: string,
    ...
    return this.partnershipsService.respondToInvitation(
      id,
      req.user,
      dto.status,
    );
  ...
  @ApiOperation({
    summary:
      'Get all partnership invitations (sent and received) with pagination',
    description:
      'Retrieves all invitations involving the branch, filterable by sent/received and status.',
  })

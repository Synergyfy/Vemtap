...
import { ChevronDown, ChevronRight, ExternalLink, Search, BookOpen, ArrowRight, Lightbulb } from 'lucide-react';
import { buildSeedDocs } from '@/constants/knowledgeBaseDocs';
import type { ContentBlock, KBCategory } from '@/constants/knowledgeBaseDocs';
import { useKnowledgeBasePage, useKnowledgeBaseTree } from '@/services/knowledge-base/hooks';

type DocPage = { id: string; title: string; path: string; summary: string; thumbnail?: string; blocks: ContentBlock[]; tips?: string[] };
type DocSection = { id: string; title: string; pages: DocPage[] };
type DocCategory = { id: string; title: string; sections: DocSection[] };

export default function TutorialDocsPage({ title = 'Knowledge Base' }: { title?: string }) {
    const { data: treeData, isError, isLoading } = useKnowledgeBaseTree();
    const apiMode = !!treeData;

    const docs = useMemo<DocCategory[]>(() => {
        if (isError) return buildSeedDocs();
        if (!treeData) return [];
        return (treeData.categories ?? []).map((cat) => ({
            id: cat.id,
            title: cat.title,
    ...
    const all = useMemo(
        () => docs.flatMap((cat) => cat.sections.flatMap((sec) => sec.pages.map((page) => ({ cat, sec, page: page as DocPage })))),
        [docs]
    );
    ...
    const pageSearchText = (page: DocPage) =>
    ...
    const { data: fullPage, isFetching } = useKnowledgeBasePage(apiMode && active?.page.id ? active.page.id : undefined);

    const activeContent = apiMode ? fullPage : active?.page;
        ...
        return (
            <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white flex items-center justify-center">
                <div className="text-center">
                    <BookOpen size={32} className="mx-auto text-gray-300 mb-3 animate-spin" />
    ...
    const openPageHref = active?.page.path
        ? active.page.path.startsWith('/')
            ? active.page.path
            : `/tutorial/bussiness?path=${encodeURIComponent(active.page.path)}`
        : '#';
                        ...
                        {!active ? (
                            ...
                            <article className="rounded-2xl border border-gray-200 bg-white shadow-sm overflow-hidden">
                                ...
                                <div className="p-6 sm:p-8">
                                    ...
                                    {apiMode && isFetching ? (
                                        <div className="py-16 text-center text-sm text-gray-400">Loading article&hellip;</div>
                                    ) : (
                                        <div className="prose prose-gray max-w-none">
                                            {renderBlocks(activeContent?.blocks ?? [])}
                                        </div>
                                    )}

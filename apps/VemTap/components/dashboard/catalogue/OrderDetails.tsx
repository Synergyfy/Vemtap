                ...
                <div className="flex flex-col md:flex-row items-center md:items-start gap-8">
                    ...
                    <div className="text-center md:text-left">
                        <div className="flex flex-col md:flex-row md:items-center gap-3 mb-2">
                            <h1 className="text-4xl font-black text-gray-900 uppercase">{order.ref}</h1>
            ...
            <div className="flex flex-col md:flex-row items-center md:items-start justify-between gap-8">
                <div className="flex flex-col md:flex-row items-center md:items-start gap-6">
                    ...
                    <div className="text-center md:text-left">
                        ...
                        <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mt-1">Loyalty Member • 24 Visits</p>
                        ...
                        <div className="mt-6 flex flex-wrap justify-center md:justify-start gap-3">
                            {[
                                ...
                                <button key={act.label} className={cn("size-12 rounded-xl flex items-center justify-center shadow-sm transition-all hover:scale-110 active:scale-95", act.color)}>
                                    <act.icon size={20} />
                                </button>
                            ))}
                        </div>
                ...
                <Link href={`/dashboard/visitors/${customer.id}`}>
                    <Button variant="outline" className="h-12 px-6 rounded-2xl border-gray-100 font-black text-[10px] uppercase tracking-widest text-gray-400 hover:text-[#066CF4]">
    ...
    return (
        <div className="rounded-[40px] bg-white p-8 shadow-sm border border-gray-100 mb-8">
                ...
                {items.map((item, i) => (
                    <div key={i} className="flex items-center justify-between p-6 rounded-3xl bg-gray-50 border border-gray-50 group hover:bg-white hover:border-gray-100 transition-all">
                        <div className="flex items-center gap-4">
                            <div className="size-16 rounded-2xl bg-white shadow-sm overflow-hidden flex items-center justify-center shrink-0">
            ...
            <div className="mt-10 pt-8 border-t border-gray-50 flex justify-between items-center">
                ...
                <span className="text-4xl font-black text-gray-900">₦12,500</span>
            </div>
        </div>
    );
}
...
export function OrderTimeline({ status }: { status: string }) {
    const steps = [
        { label: 'Request Submitted', time: '10:45 AM', active: true },
        { label: 'Order Confirmed', time: '10:48 AM', active: status !== 'Pending' },
...
export function OrderManagementActions({ status, onStatusChange }: any) {
    return (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-[50] w-[calc(100%-48px)] max-w-2xl bg-gray-900 p-4 rounded-[32px] shadow-2xl flex items-center gap-4 md:bottom-10">
            ...
            <div className="flex-1 flex gap-3">
                {status === 'Pending' ? (
                    <Button onClick={() => onStatusChange('Confirmed')} className="h-14 flex-1 rounded-2xl bg-[#066CF4] text-[10px] font-black uppercase tracking-widest text-white active:scale-95 transition-all">
                        Confirm Request
                    </Button>
                ) : (
                    <Button onClick={() => onStatusChange('Completed')} className="h-14 flex-1 rounded-2xl bg-emerald-500 text-[10px] font-black uppercase tracking-widest text-white active:scale-95 transition-all">
                        Mark Completed
                    </Button>
                )}
                <Button variant="ghost" className="h-14 px-6 rounded-2xl bg-white/5 text-gray-400 hover:text-red-400 hover:bg-red-400/10">
                    <XCircle size={20} />
                </Button>
            </div>
        </div>

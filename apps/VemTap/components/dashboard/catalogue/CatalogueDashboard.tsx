...
export function CatalogueStatsCards({ stats }: { stats: any[] }) {
    return (
        <div className="flex gap-4 overflow-x-auto no-scrollbar -mx-6 px-6 snap-x">
            {stats.map((stat, i) => (
                <Link key={i} href={stat.href || "/dashboard/inventory"} className="min-w-[240px] md:flex-1 snap-center group">
                    <motion.div 
                        initial={{ opacity: 0, y: 10 }}
                        animate={{ opacity: 1, y: 0 }}
                        transition={{ delay: i * 0.1 }}
                        className="rounded-[2.5rem] bg-white p-6 shadow-sm border border-gray-100 flex flex-col justify-between h-40 group-hover:border-[#066CF4]/20 transition-all h-full cursor-pointer"
                    >

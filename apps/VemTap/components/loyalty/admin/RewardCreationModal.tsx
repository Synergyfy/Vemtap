            ...
            <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-8">
                ...
                >
                    ...
                    <div className="p-8 bg-slate-50 border-t border-slate-100 flex items-center justify-between">
                        <button onClick={onClose} className="text-sm font-semibold text-slate-400">Cancel</button>
                        <button
                            onClick={handleSubmit}
                            disabled={isUploading}
                            className="px-10 py-4 bg-primary text-white font-semibold text-sm rounded-2xl shadow-xl shadow-primary/20 disabled:opacity-70 flex items-center gap-2"
                        >
                            {isUploading && <Loader2 size={16} className="animate-spin" />}
                            {initialData ? 'Update Reward' : 'Confirm & Launch'}
                        </button>
                    </div>
                </motion.div>
            </div>

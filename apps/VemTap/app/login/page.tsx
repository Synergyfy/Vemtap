...
function LoginPageContent() {
    ...
    const handleGoogleSignIn = () => {
        // Use google.accounts.id directly to avoid the opacity-0 cross-origin
        // iframe click-blocking issue in modern browsers on production HTTPS.
        const google = (window as any).google;
        if (!google?.accounts?.id) {
            setGeneralError('Google Sign-In is not available. Please refresh the page.');
            return;
        }
        ...
        google.accounts.id.prompt((notification: any) => {
            if (notification.isNotDisplayed() || notification.isSkippedMoment()) {
                ...
                setGeneralError('Google sign-in prompt was blocked. Please try again or clear your browser cookies.');
            }
        });
    };
                ...
                <div className="max-w-md w-full mx-auto">
                    ...
                    <div className="mt-8">
                        ...
                        <button
                            ...
                            disabled={isLoggingIn || isGooglePrompting}
                            className="w-full h-12 rounded-xl border border-gray-200 font-medium text-sm text-text-main flex items-center justify-center gap-3 hover:bg-gray-50 transition-all cursor-pointer disabled:opacity-60 disabled:cursor-not-allowed"
                        >
                            {isGooglePrompting ? (

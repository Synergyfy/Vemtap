...
import { 
  ...
  Save, Edit2, Loader2, Info, ArrowLeft
} from 'lucide-react';
import { notify } from '@/lib/notify';

export default function AdminProfilingResultScreen() {
  ...
  if (error || !profile) {
    return (
      <div className="flex h-screen flex-col items-center justify-center bg-gray-50">
        ...
        <button onClick={() => router.push('/admin/business-profiling')} className="mt-6 px-4 py-2 bg-primary text-white rounded-xl hover:bg-primary/90 transition-colors">
          Go Back
        </button>
      </div>
    );
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 p-4 shadow-[0_-10px_40px_-15px_rgba(0,0,0,0.1)] z-50 md:sticky md:bottom-auto md:mt-8 md:bg-transparent md:border-none md:shadow-none md:p-0">
        <div className="mx-auto max-w-7xl md:px-8">
          <div className="flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-end">
            <button onClick={() => router.push(`/admin/business-profiling`)} className="flex-1 sm:flex-none inline-flex items-center justify-center rounded-xl px-4 py-2.5 text-sm font-semibold border-2 border-gray-200 text-gray-700 hover:bg-gray-50 hover:border-gray-300 transition-all">

...
const MOCK_CHATS = [
    {
        id: '1',
        user: { name: 'Alex Morgan', email: 'alex@startup.io', avatar: 'AM', status: 'Online' },
        lastMessage: 'I have a question about the NFC tag range...',
    ...
    {
        id: '2',
        user: { name: 'Sarah Chen', email: 'sarah.c@retail.com', avatar: 'SC', status: 'Offline' },
        lastMessage: 'The new dashboard layout looks amazing!',
    ...
    {
        id: '3',
        user: { name: 'James Wilson', email: 'j.wilson@corp.net', avatar: 'JW', status: 'Online' },
        lastMessage: 'Can we schedule a demo for the loyalty program?',
...
const MOCK_MESSAGES: Record<string, any[]> = {
    '1': [
        { id: 'm1', sender: 'user', text: 'Hi there! I just received my NFC tags.', timestamp: '10:30 AM' },
        { id: 'm2', sender: 'agent', text: 'Great to hear, Alex! How are they working out for you?', timestamp: '10:32 AM' },
        { id: 'm3', sender: 'user', text: 'They are great, but I have a question about the range. Is it normal to need 1-2cm proximity?', timestamp: '10:35 AM' }
    ]
};

export default function AdminChatDesk() {
    const [activeId, setActiveId] = useState<string | null>(MOCK_CHATS[0].id);
    const [searchQuery, setSearchQuery] = useState('');
    const [replyText, setReplyText] = useState('');
    const [isSidebarOpen, setIsSidebarOpen] = useState(true);
    
    const activeChat = useMemo(() => MOCK_CHATS.find(c => c.id === activeId), [activeId]);
    const messages = useMemo(() => (activeId ? MOCK_MESSAGES[activeId] || [] : []), [activeId]);
                        ...
                        >
                            <div className="relative">
                                ...
                                <div className={cn(
                                    ...
                                    chat.user.status === 'Online' ? 'bg-emerald-500' : 'bg-gray-300'
                                )} />
                            </div>
                            <div className="flex-1 min-w-0">
                                <div className="flex justify-between items-start mb-0.5">
                                    <p className="font-bold text-[15px] text-gray-900 truncate">{chat.user.name}</p>
                                ...
                                <div className="flex items-center gap-2 mt-2">
                                    <span className={cn(
                                        ...
                                        chat.status === 'Active' ? 'bg-blue-50 text-blue-600' : 
                                        chat.status === 'Waiting' ? 'bg-amber-50 text-amber-600' : 'bg-emerald-50 text-emerald-600'
                                    )}>
                                        {chat.status}
                                    </span>
                                    {chat.unreadCount > 0 && (
                                        <span className="bg-red-500 text-white text-[10px] h-4 min-w-[16px] px-1 rounded-full flex items-center justify-center font-black animate-bounce">
                                            {chat.unreadCount}
                                        </span>
                                    )}
                                </div>
                            ...
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <div className="flex items-center gap-2">
                                        <h3 className="font-bold text-gray-900 text-lg leading-none">{activeChat.user.name}</h3>
                                        <div className="w-2 h-2 bg-emerald-500 rounded-full"></div>
                                    </div>
                                    <p className="text-[10px] text-gray-400 font-bold uppercase tracking-[0.2em] mt-1">
                                        Active Conversation • {activeChat.priority} Priority
                                    </p>
                                </div>
                            </div>
                            <div className="flex items-center gap-2">
                                ...
                                <button className="bg-emerald-500 hover:bg-emerald-600 text-white px-6 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all shadow-lg shadow-emerald-500/20 active:scale-95">
                                    Resolve
                                </button>
                            </div>
                        </div>
                                ...
                                return (
                                    ...
                                    >
                                        <div className={cn(
                                            ...
                                            {isAgent ? 'YOU' : activeChat.user.avatar}
                                        </div>
                                        <div className={cn(
                        ...
                        <div className="p-8 border-t border-gray-50 bg-white">
                            <form onSubmit={handleSendMessage} className="relative group">
                                ...
                                <textarea
                                    ...
                                    placeholder={`Reply to ${activeChat.user.name}...`}
                    ...
                    >
                        <div className="p-8 space-y-8 overflow-y-auto">
                            <div className="text-center">
                                ...
                                <h4 className="text-lg font-bold text-gray-900 leading-tight">{activeChat.user.name}</h4>
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="space-y-3">
                                    {[
                                        ...
                                        { icon: Clock, label: 'Timezone', value: 'GMT-8 (Pacific)' }
                                    ].map((item, idx) => (

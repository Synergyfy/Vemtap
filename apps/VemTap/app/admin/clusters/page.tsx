...
import { Plus, Loader2, MapPin, Zap, RotateCcw } from 'lucide-react';
...
import ClusterRotatorDrawer from '@/components/admin/clusters/rotator/ClusterRotatorDrawer';

type PaneView = 'map' | 'rotation';
...
export default function AdminClusterManagementPage() {
    ...
    const [view, setView] = useState<PaneView>('map');
    ...
    // Deep-link support: /admin/clusters?cluster=<id>&view=rotation
    useEffect(() => {
        const params = new URLSearchParams(window.location.search);
        const clusterId = params.get('cluster');
        if (clusterId) setSelectedClusterId(clusterId);
        if (params.get('view') === 'rotation') {
            if (clusterId) setRotationOpen(true);
            setView('rotation');
        }
    }, []);
    ...
    const openRotation = useCallback((clusterId: string) => {
    ...
    const showRotationPane = view === 'rotation' && isDesktop;

    return (
        <div className="h-[calc(100vh-4rem)] flex flex-col">
            <div className="shrink-0 px-6 py-4 bg-white border-b border-gray-100 flex items-center justify-between">
                <div>
                    <h1 className="text-lg font-display font-bold text-text-main">
                        {showRotationPane ? 'Cluster Deal Rotation' : 'Cluster Management'}
                    </h1>
                    <p className="text-xs text-text-secondary">
                        {showRotationPane
                            ? 'Control eligibility, strategy, scheduling and frequency for this cluster.'
                ...
                <div className="flex items-center gap-2">
                    {showRotationPane ? (
                        <button
                            onClick={() => setView('map')}
                            className="flex items-center gap-2 px-4 py-2 bg-white border border-gray-200 text-text-secondary rounded-xl text-xs font-black uppercase tracking-widest hover:border-blue-300 hover:text-blue-600 transition-all"
                        >
                            <RotateCcw size={14} />
                            Back to Map
                        </button>
                            ...
                            <div className="absolute inset-0 flex items-center justify-center">
                                <div className="text-center max-w-sm">
                                    ...
                                    <p className="text-sm text-text-secondary mt-1">
                                        Pick a cluster on the left to open its deal rotation page.
                                    </p>
                                </div>
                            </div>
                        )
                        ...
                        <>
                            ...
                            {selectedCluster && (
                                <ClusterMapOverlay
                                    ...
                                    onRotation={() => openRotation(selectedCluster.id)}
            ...
            {/* Mobile / tablet rotator drawer */}
            <ClusterRotatorDrawer
                key={selectedClusterId ?? 'none'}
                open={rotationOpen && !!selectedCluster}
                cluster={selectedCluster}

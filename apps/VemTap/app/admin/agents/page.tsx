...
import InviteAgentModal from '@/components/admin/agents/InviteAgentModal';
...
export default function AdminAgentsPage() {
    ...
    const { data: userData, isLoading, refetch } = useQuery({
        ...
        queryFn: () => adminUsersApi.getAll({ limit: 100, role: 'Agent' }),
    });
    ...
    const staffList = (userData?.data || userData?.users || userData || []).filter((u: any) => u.role === 'Agent');
    ...
    const handleInvite = async (data: any) => {
        ...
        try {
            ...
            notify.success(`Agent ${data.name} created and activated!`);
            queryClient.invalidateQueries({ queryKey: ['admin-agents'] });
            refetch(); // Ensure the list is immediately updated
        } catch (err: any) {

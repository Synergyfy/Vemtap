...
import { Search, Filter, Calendar, CreditCard, Banknote, FileText, ArrowRight } from 'lucide-react';
import { cn } from '@/lib/utils';
import { motion } from 'framer-motion';

export default function SalesDashboard() {
  const router = useRouter();
  const { completedSales, seedSales, isSeeded, getTodaysRevenue, getTodaysTransactionCount } = usePosStore();
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-[#066CF4] text-white rounded-[32px] p-6 md:p-8 shadow-xl shadow-blue-500/20 relative overflow-hidden">
          ...
          <div className="relative z-10">
            ...
            <h3 className="text-3xl md:text-4xl font-black mb-1">₦{getTodaysRevenue().toLocaleString()}</h3>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="bg-white border border-gray-100 rounded-[32px] p-6 md:p-8 shadow-sm flex flex-col justify-center">
          <p className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-400 mb-2">Transactions Today</p>
          <h3 className="text-2xl md:text-3xl font-black text-gray-900 mb-1">{getTodaysTransactionCount()}</h3>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="bg-white border border-gray-100 rounded-[32px] p-6 md:p-8 shadow-sm flex flex-col justify-center">
          ...
          <h3 className="text-2xl md:text-3xl font-black text-gray-900 mb-1">
            ₦{getTodaysTransactionCount() > 0 ? Math.round(getTodaysRevenue() / getTodaysTransactionCount()).toLocaleString() : 0}
          </h3>
                ...
                {filteredSales.map(sale => (
                  ...
                  >
                    ...
                    <td className="p-4 hidden sm:table-cell">
                      <p className="text-xs font-bold text-gray-900">{sale.customer ? sale.customer.name : 'Walk-in'}</p>
                    ...
                    <td className="p-4 hidden md:table-cell">
                      <div className="flex items-center gap-2 text-gray-600">
                        {sale.payment.method === 'cash' ? <Banknote size={14} /> : <CreditCard size={14} />}
                        <span className="text-[10px] font-black uppercase tracking-widest">{sale.payment.method}</span>
                      </div>
                    </td>

...
import { ShoppingBag, CheckCircle, Loader2, ChevronRight, Phone, User, Package, ArrowLeft, AlertCircle, X, BarChart3, TrendingUp, RotateCcw, MessageSquare } from 'lucide-react';
        ...
        return (
          <div className="flex gap-2">
            ...
            <button
              ...
              disabled={updateStatus.isPending}
              className="h-12 px-4 bg-red-50 text-red-600 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-red-100 transition-all disabled:opacity-50 flex items-center gap-1.5"
            >
              <RotateCcw size={14} />
              Return
            </button>
        ...
        return (
          <div className="flex gap-2">
            <button
              ...
              disabled={updateStatus.isPending}
              className="flex-1 h-12 bg-emerald-500 text-white rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-emerald-600 transition-all disabled:opacity-50 flex items-center justify-center gap-2"
            >
              {updateStatus.isPending ? <Loader2 size={14} className="animate-spin" /> : <CheckCircle size={16} />}
              Mark Complete
            </button>
      ...
      case 'completed':
        return (
          <button
            onClick={openRefundModal}
            disabled={updateStatus.isPending}
            className="w-full h-12 bg-red-50 text-red-600 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-red-100 transition-all disabled:opacity-50 flex items-center justify-center gap-2"
          >
            <RotateCcw size={16} />
            Return & Refund
          </button>
        ...
        actions={
          stats.new > 0 && (
            <div className="flex items-center gap-2 px-4 py-2 bg-blue-50 border border-blue-100 rounded-xl">
              ...
              <span className="text-[10px] font-black uppercase tracking-widest text-blue-600">{stats.new} new</span>
            </div>
          )
        }
      />
            ...
            >
              <div className="p-6 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-lg font-black text-gray-900">Return & Refund</h3>
                    <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Select items to refund</p>
                  </div>
                </div>
                <button onClick={() => setShowRefundModal(false)} className="size-8 rounded-lg bg-white border border-gray-200 flex items-center justify-center text-gray-400 hover:text-gray-600 transition-all">
                  <X size={16} />
                </button>
              </div>
              ...
              <div className="p-6 space-y-5 max-h-[60vh] overflow-y-auto">
                ...
                <div className="space-y-2">
                  ...
                  {(orderDetail.items || []).map((item: any) => (
                    ...
                    >
                      <input
                        type="checkbox"
                        checked={refundItemIds.has(item.id)}
                        onChange={() => toggleRefundItem(item.id)}
                        className="size-5 accent-red-500 rounded-lg"
                      />
                      <div className="flex-1">
                        <p className="text-sm font-black text-gray-900">{item.item?.name || item.offer?.name || 'Item'}</p>
                        <p className="text-[10px] font-bold text-gray-400">Qty: {item.quantity} × ₦{Number(item.priceAtOrder).toLocaleString()}</p>
                      </div>
                ...
                <div className="bg-gray-50 rounded-2xl p-4 flex justify-between items-center">
                  ...
                  <span className="text-lg font-black text-red-500">
                    ₦{(orderDetail.items || [])
                      .filter((i: any) => refundItemIds.has(i.id))
                      .reduce((sum: number, i: any) => sum + Number(i.priceAtOrder) * i.quantity, 0)
                      .toLocaleString()}
                  </span>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <textarea
                    ...
                    placeholder="Explain why this order/items are being returned..."
                    rows={3}
                    className="w-full p-4 bg-gray-50 border border-gray-200 rounded-2xl text-sm font-bold outline-none resize-none focus:border-red-300 focus:ring-4 focus:ring-red-500/10"
                  />
                </div>
              </div>
              ...
              <div className="p-6 border-t border-gray-100 flex gap-3">
                <button
                  onClick={() => setShowRefundModal(false)}
                  className="flex-1 h-12 bg-gray-50 text-gray-600 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-gray-100 transition-all"
                >
                  Cancel
                </button>
                <button
                  onClick={handleRefundConfirm}
                  disabled={updateStatus.isPending || refundItemIds.size === 0}
                  className="flex-1 h-12 bg-red-500 text-white rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-red-600 transition-all disabled:opacity-40 flex items-center justify-center gap-2"
                >
                  {updateStatus.isPending ? <Loader2 size={14} className="animate-spin" /> : <RotateCcw size={14} />}
                  Confirm Refund
                </button>
              </div>
            </motion.div>

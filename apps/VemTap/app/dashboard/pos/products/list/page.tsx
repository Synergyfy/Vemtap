...
export default function ProductsList() {
  ...
  const { products, categories, getLowStockProducts } = useProductStore();
  ...
  const filteredProducts = products.filter(p => {
    ...
    if (activeTab === 'low_stock') return matchesSearch && p.status === 'low_stock';
    if (activeTab === 'out_of_stock') return matchesSearch && p.status === 'out_of_stock';
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="p-4 text-right">
                        <p className="text-sm font-black text-gray-900">₦{product.sellingPrice.toLocaleString()}</p>
                        <p className="text-[9px] font-bold text-gray-400 mt-0.5">Cost: ₦{product.costPrice.toLocaleString()}</p>
                      </td>
                      <td className="p-4 text-right">
                      ...
                      <td className="p-4 text-right">
                        <button className="p-2 rounded-xl hover:bg-gray-200 text-gray-400 transition-colors" onClick={(e) => { e.stopPropagation(); /* Menu */ }}>

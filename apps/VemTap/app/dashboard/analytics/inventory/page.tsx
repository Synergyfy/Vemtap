    ...
    <div className="max-w-5xl mx-auto h-full flex flex-col pt-4 px-4 md:px-0 pb-24">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 mb-6">
        {kpis.map((kpi, i) => (
          ...
          >
            <div className={`size-9 md:size-10 rounded-lg flex items-center justify-center mb-3 ${kpi.bg} ${kpi.color}`}>
              <kpi.icon size={18} />
            </div>
      ...
      {/* Inventory Table */}
      <div className="bg-white border border-gray-100 rounded-2xl p-5 md:p-6 shadow-sm flex-1">
        <h3 className="text-[10px] font-semibold uppercase tracking-wider text-gray-400 mb-4">
        ...
        {products.length > 0 ? (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-100">
                  <th className="text-left text-[10px] font-semibold text-gray-400 uppercase tracking-wider pb-3">Product</th>
                  <th className="text-right text-[10px] font-semibold text-gray-400 uppercase tracking-wider pb-3">Price</th>
                  <th className="text-right text-[10px] font-semibold text-gray-400 uppercase tracking-wider pb-3">Qty</th>
                  <th className="text-right text-[10px] font-semibold text-gray-400 uppercase tracking-wider pb-3">Value</th>
                  <th className="text-center text-[10px] font-semibold text-gray-400 uppercase tracking-wider pb-3">Status</th>
                </tr>
              </thead>
                ...
                {sortedProducts.slice(0, 20).map((p: any, i: number) => {
                  ...
                  return (
                    <tr key={p.id ?? i} className="hover:bg-gray-50/50 transition-colors">
                      ...
                      <td className="py-3 text-right font-bold text-gray-900">{qty}</td>
                      <td className="py-3 text-right font-semibold text-gray-700">₦{value.toLocaleString()}</td>
                      <td className="py-3 text-center">
                        <span className={`text-[10px] font-semibold uppercase tracking-wider px-2 py-1 rounded-lg ${statusColor}`}>
                          {statusLabel}
                        </span>
                      </td>
                    </tr>
            ...
            {products.length > 20 && (
              <p className="text-center text-[10px] font-semibold text-gray-400 uppercase tracking-wider mt-4">
          ...
          <div className="flex flex-col items-center justify-center min-h-[200px] text-gray-300">
            ...
            <p className="text-xs font-semibold uppercase tracking-wider">Start tracking your inventory to see insights here</p>
            {!activeBranchId && (

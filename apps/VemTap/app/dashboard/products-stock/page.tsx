...
export default function ProductsAndStockDashboard() {
  ...
  const { products, getProductStats, getTotalInventoryValue, isSeeded: isProdSeeded, seedProducts } = useProductStore();
  const { isSeeded: isInvSeeded, seedMovements } = useInventoryStore();
  ...
  const lowStockProducts = products.filter(p => p.quantity > 0 && p.quantity <= (p.minStock || 5)).slice(0, 5);
    ...
    <div className="p-4 md:p-6 lg:p-8 max-w-7xl mx-auto space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="bg-white border border-gray-100 rounded-[32px] p-6 shadow-sm flex flex-col justify-center">
          ...
          <h3 className="text-2xl font-black text-gray-900 mb-1">{stats.active} Active</h3>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="bg-white border border-gray-100 rounded-[32px] p-6 shadow-sm flex flex-col justify-center">
          <div className="flex justify-between items-start">
            <div>
              ...
              <h3 className="text-2xl font-black text-red-500 mb-1">{stats.outOfStock + stats.lowStock}</h3>
              <p className="text-xs font-bold text-gray-500">Low or Out of Stock</p>
            </div>
            <AlertTriangle className={stats.outOfStock > 0 ? "text-red-500" : "text-amber-500"} size={24} />
          </div>
        </motion.div>
      </div>
              ...
              <div className="space-y-2">
                {lowStockProducts.map(p => (
                  <div key={p.id} className="flex justify-between items-center bg-white p-3 rounded-2xl">
                    ...
                    <span className="text-xs font-black text-amber-600">{p.quantity} remaining</span>
                  </div>
                ))}
              </div>
            </div>
                ...
                <div key={p.id} className="flex items-center gap-4 p-3 rounded-2xl hover:bg-gray-50 transition-colors border border-transparent hover:border-gray-100">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">
                      {p.category} • ₦{p.sellingPrice.toLocaleString()}
                    </p>
                  </div>
                  <div className="text-right shrink-0">
                    <p className="text-xs font-black text-gray-900">{p.quantity} in stock</p>
                  </div>
                </div>
              ))}

      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6">
        <div className="space-y-4 md:space-y-6">
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 md:p-6">
            <h3 className="text-[10px] font-semibold uppercase tracking-wider text-gray-400 mb-4">Quick Actions</h3>
            <div className="grid grid-cols-2 gap-3">
              <button
                onClick={() => router.push('/dashboard/catalogue/products')}
                className="flex flex-col items-center justify-center gap-3 p-4 rounded-xl border transition-all active:scale-95 text-blue-500 bg-blue-50 border-blue-100"
              >
                <Plus size={24} />
                <span className="text-[10px] font-semibold uppercase tracking-wider text-gray-900">Add Product</span>
              </button>
              <button
                onClick={() => router.push('/dashboard/inventory/receiving')}
                className="flex flex-col items-center justify-center gap-3 p-4 rounded-xl border transition-all active:scale-95 text-emerald-500 bg-emerald-50 border-emerald-100"
              >
                <ArrowDownToLine size={24} />
                <span className="text-[10px] font-semibold uppercase tracking-wider text-gray-900">Receive Stock</span>
              </button>
            </div>
            ...
            <div className="mt-6 space-y-3">
              <button
                ...
                className="w-full h-12 bg-gray-50 hover:bg-gray-100 text-gray-900 rounded-xl flex items-center justify-between px-4 transition-colors"
              >
                <div className="flex items-center gap-3">
                  <Box size={16} className="text-gray-400" />
                  <span className="text-xs font-semibold uppercase tracking-wider">View Catalogue</span>
                </div>
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-xs font-semibold uppercase tracking-wider">Manage Inventory</span>
                </div>
                <ArrowRight size={16} className="text-gray-400" />
              </button>
            </div>
        ...
        <div className="lg:col-span-2 space-y-4 md:space-y-6">
            ...
            <div className="bg-amber-50 rounded-2xl border border-amber-100 p-5 md:p-6">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2 text-amber-700">
                  ...
                  <h3 className="text-sm font-bold">Low Stock Items</h3>
                </div>
                <button onClick={() => router.push('/dashboard/inventory/low-stock')} className="text-[10px] font-semibold uppercase tracking-wider text-amber-700 hover:underline">
                  View All
                </button>
              </div>
              <div className="space-y-2">
                {lowStockProducts.map((p: any) => (
                  <div key={p.id} className="flex justify-between items-center bg-white p-3 rounded-xl">
                    <span className="text-xs font-semibold text-gray-900">{p.name}</span>
                    <span className="text-xs font-bold text-amber-600">{p.stockQuantity} remaining</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 md:p-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-sm font-bold text-gray-900">Recently Added Products</h3>
              <button onClick={() => router.push('/dashboard/pos/products')} className="text-[10px] font-semibold uppercase tracking-wider text-[#066CF4] hover:underline">
                All Products
              </button>
            </div>
            <div className="space-y-2">
              {recentProducts.map((p: any) => (
                <div key={p.id} className="flex items-center gap-4 p-3 rounded-xl hover:bg-gray-50 transition-colors border border-transparent hover:border-gray-100">
                  ...
                  <div className="text-right shrink-0">
                    <p className="text-xs font-bold text-gray-900">{p.stockQuantity} in stock</p>
                  </div>
                </div>
              ))}
              {recentProducts.length === 0 && (
                <div className="text-center py-8 text-gray-400">
                  <p className="text-xs font-semibold uppercase tracking-wider">Ready to stock your inventory? Add your first product.</p>
                </div>
              )}
            </div>
          </div>

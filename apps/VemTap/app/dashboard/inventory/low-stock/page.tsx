        ...
        {alerts.length > 0 ? (
          <div className="space-y-4">
            {alerts.map((item: any) => {
              ...
              return (
                <div key={item.id} className="p-4 md:p-6 bg-gray-50 rounded-xl border border-gray-100 flex flex-col md:flex-row gap-4 items-start md:items-center justify-between hover:bg-white hover:shadow-md transition-all">
                  ...
                  <div className="flex items-center gap-6 w-full md:w-auto mt-2 md:mt-0 pt-4 md:pt-0 border-t border-gray-200 md:border-0">
                    <div className="flex-1 md:flex-none">
                      <p className="text-[9px] font-semibold uppercase tracking-wider text-gray-400 mb-1">Current Stock</p>
                      <p className={cn("text-xl font-bold", isOut ? "text-red-500" : "text-amber-500")}>
                        {item.stockQuantity}
                      </p>
                    </div>
                    <div className="flex-1 md:flex-none">
                      <p className="text-[9px] font-semibold uppercase tracking-wider text-gray-400 mb-1">Min Threshold</p>
                      <p className="text-xl font-bold text-gray-900">{item.minStock || 5}</p>
                    </div>

                    <button
                      onClick={() => router.push(`/dashboard/inventory/receiving?product=${item.id}`)}
                      className="h-10 px-4 rounded-xl bg-[#066CF4] text-white flex items-center gap-2 hover:bg-blue-600 transition-colors shadow-sm shadow-blue-500/20"
                    >
                      <ArrowDownToLine size={16} />
                      <span className="text-[10px] font-semibold uppercase tracking-wider hidden sm:inline">Reorder</span>
                    </button>
                  </div>
                </div>
              );
          ...
          <div className="h-full flex flex-col items-center justify-center text-center text-emerald-500">
            <div className="size-20 bg-emerald-50 rounded-2xl flex items-center justify-center mb-6">
              <Package size={40} />
            </div>
            <h2 className="text-2xl md:text-3xl font-bold text-gray-900 mb-2">Stock Levels Healthy</h2>
            <p className="text-[11px] font-semibold text-gray-500 uppercase tracking-wider max-w-sm">

'use client';

import React from 'react';
import POSPageHeader from '@/components/dashboard/pos/shared/POSPageHeader';
import { Activity } from 'lucide-react';

export default function StaffActivityScreen() {
  return (
    <div className="max-w-5xl mx-auto h-full flex flex-col pt-4 px-4 md:px-0 pb-24">
      <POSPageHeader 
        title="Staff Activity Log" 
        subtitle="Audit trail of all actions"
      />

      <div className="flex-1 flex flex-col items-center justify-center text-center p-8 bg-white border border-gray-100 rounded-[32px] shadow-sm">
        <div className="size-24 bg-gray-50 rounded-[32px] flex items-center justify-center mb-6 border border-gray-200">
          <Activity size={48} className="text-gray-400" />
        </div>
        <h2 className="text-2xl font-black text-gray-900 mb-2">Audit Log</h2>
        <p className="text-[11px] font-bold text-gray-500 uppercase tracking-widest max-w-sm leading-relaxed mb-8">
          A tamper-proof log of every action taken in the system: logins, voided sales, applied discounts, and manual drawer openings.
        </p>
        
        <p className="text-[10px] font-black uppercase tracking-widest text-gray-600 bg-gray-100 px-4 py-2 rounded-lg">
          Security Feature - Phase 5b
        </p>
      </div>
    </div>
  );
}

...
export default function CustomerNotificationsPage() {
    ...
    return (
        <div className="max-w-4xl mx-auto space-y-5 md:space-y-8 pb-20 p-4 md:p-0">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div className="flex items-center gap-3">
                    <Link
                        ...
                        className="w-10 h-10 rounded-xl bg-white border border-gray-200 flex items-center justify-center text-text-secondary hover:text-primary hover:border-primary/30 transition-all shadow-sm group"
                    >
                        <ArrowLeft size={18} className="group-hover:-translate-x-1 transition-transform" />
                    </Link>
                    <div>
                        <h1 className="text-2xl md:text-3xl font-display font-bold text-text-main tracking-tight">Alert Center</h1>
                        <p className="text-sm text-text-secondary font-medium mt-0.5">Updates on your rewards, visits, and special offers.</p>
                    </div>
                </div>
                ...
                {isLoading ? (
                    <div className="p-16 flex flex-col items-center justify-center gap-5">
                        <div className="relative">
                            ...
                            <div className="animate-spin rounded-full h-10 w-10 border-4 border-primary border-t-transparent shadow-lg shadow-primary/20 relative"></div>
                        </div>
                    ...
                    <div className="p-12 flex flex-col items-center justify-center text-center">
                        <div className="w-20 h-20 bg-gray-50 rounded-2xl flex items-center justify-center mb-5 shadow-inner">
                            <Bell className="text-gray-300" size={32} />
                        </div>
                        <h3 className="text-lg font-display font-bold text-text-main mb-1.5">Peace & Quiet</h3>
                    ...
                    <div className="divide-y divide-gray-50">
                        {notifications.map((note: Notification) => (
                            <div
                                ...
                                className={`p-4 flex items-start gap-4 hover:bg-gray-50/50 transition-all cursor-pointer relative group ${!note.read ? 'bg-primary/2' : ''}`}
                            >
                                ...
                                <div className={`w-11 h-11 rounded-xl flex items-center justify-center shrink-0 shadow-sm transition-transform group-hover:scale-110 duration-500 ${!note.read ? 'bg-white border border-primary/20' : 'bg-gray-50 grayscale'
                                    }`}>
                                ...
                                <div className="flex-1 min-w-0">
                                    <div className="flex items-center justify-between gap-3 mb-1.5">
                                        <h4 className={`text-sm truncate ${!note.read ? 'font-display font-bold text-text-main' : 'text-text-secondary font-medium'}`}>
                                            {note.title}
                                        </h4>
            ...
            {/* Engagement Card */}
            <div className="bg-linear-to-r from-primary to-blue-600 rounded-2xl p-5 text-white relative overflow-hidden shadow-2xl shadow-primary/20">
